...
<title>Каталог книг - Sergio Books</title>
<link rel="stylesheet"
		...
		<div id="main-block" class="row">
			...
			<div class="books col-10">
				...
				<div class="chosen-category-title">
					<h3 class="chosen-category-title-text">${showName}</h3>
				</div>
					...
					<div class="form-group">
						<div class="dropdown">
							<a class="btn btn-secondary dropdown-toggle" href="#"
								...
								aria-haspopup="true" aria-expanded="false"> ${activeSortItem }
							</a>
							<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
								<c:forEach items="${sortMenuItems }" var="sortItem">
									<a class="dropdown-item"
										href='<spring:url value="/catalog?genre=${filter }&page=0&sort=${sortItem.value }"/>'>${sortItem.key }</a>
								</c:forEach>
							</div>
						</div>
					</div>
										...
										<div class="price">
											<p>
												<span class="price-sum"><fmt:formatNumber
														type="number" maxFractionDigits="2" value="${book.price}" /></span>
												руб.
											</p>
										</div>
										<c:choose>
					...
					<ul class="pagination justify-content-end">
						<c:choose>
							...
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href='<spring:url value="/catalog?genre=${filter }&page=0&sort=${sort},${direction}"/>'
									aria-label="First page" title="В начало"> <span
										aria-hidden="true"><i class="icon-double-angle-left"></i></span>
										<span class="sr-only">First page</span>
								</a></li>
								<li class="page-item "><a class="page-link"
									href='<spring:url value="/catalog?genre=${filter }&page=${page.number - 1}&sort=${sort},${direction}"/>'
						...
						<c:forEach begin="${startpage }" end="${endpage}" var="p">
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href='<spring:url value="/catalog?genre=${filter }&page=${p}&sort=${sort},${direction}"/>'>${p + 1}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							...
							<c:otherwise>
								...
								<li class="page-item"><a class="page-link"
									href='<spring:url value="/catalog?genre=${filter }&page=${page.totalPages - 1}&sort=${sort},${direction}"/>'
